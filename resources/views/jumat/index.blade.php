...
@section('body')
<h3 class="text-3xl font-medium text-gray-700">Jadwal Petugas Jumat</h3>
<div class="mt-8">
        ...
        <div class="inline-block min-w-full overflow-hidden align-middle border-b border-gray-200 shadow sm:rounded-lg">
            ...
            <table class="min-w-full">
                ...
                <tbody class="bg-white">
                    @if ($jumat->count()<=0) <tr>
                        <td class="px-6 py-4 text-center whitespace-no-wrap border-b border-gray-200" colspan="5">Tidak Ada Jadwal</td>
                        </tr>
                        @endif
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                <div class="text-sm leading-5 text-gray-900">{{ $data->tanggal }}</div>
                            </td>

                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                <div class="text-sm leading-5 text-gray-900">{{ $data->imam }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                <div class="text-sm leading-5 text-gray-900">{{ $data->khotib }}</div>
                            </td>

                            <td class="px-6 py-4 text-sm font-medium leading-5 text-right whitespace-no-wrap border-b border-gray-200">
                                <a href="{{ route('jumat.edit',$data->id) }}" class="mr-3 font-semibold text-indigo-600 hover:text-indigo-900">Ubah</a>
                                <form action="{{ route('jumat.destroy',$data->id) }}" method="post" class="inline-block">
                                    @csrf
                                    @method('DELETE')
