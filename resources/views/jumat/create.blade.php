...
<div class="flex items-center justify-center w-full h-full">
    <div class="w-full p-6 bg-white rounded-md shadow-md md:w-1/2 h-fit">
        ...
        <form method="POST" action="{{ route('jumat.store') }}" enctype="multipart/form-data">
            @csrf
            <div class="flex flex-col gap-y-6">
                <div>
                    <label class="text-gray-700" for="tamgga;">Tanggal*</label>
                    <input class="w-full mt-2 rounded-md form-input focus:border-indigo-600" type="date" name="tanggal" required autofocus value="{{ old('tanggal') }}">
                    @error('tanggal')
                ...
                <div>
                    ...
                    <input class="w-full mt-2 rounded-md form-input focus:border-indigo-600" type="text" name="imam" required autofocus value="{{ old('imam') }}">
                    @error('imam')
                    <span class="text-sm font-semibold text-red-600">{{ $message }}</span>
                    @enderror
                </div>
                <div>
                    ...
                    <input class="w-full mt-2 rounded-md form-input focus:border-indigo-600" type="text" name="khotib" required autofocus value="{{ old('khotib') }}">
                    @error('khotib')
                    <span class="text-sm font-semibold text-red-600">{{ $message }}</span>
                    @enderror
                </div>
